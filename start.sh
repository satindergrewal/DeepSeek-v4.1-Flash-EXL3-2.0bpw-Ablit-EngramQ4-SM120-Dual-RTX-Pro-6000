...
set -uo pipefail
cd "$(dirname "$0")"
[ -f serve/serve.env ] && { set -a; . serve/serve.env; set +a; }
...
for i in $(seq 1 240); do
  curl -sf -m 5 ${API_KEY:+-H "Authorization: Bearer $API_KEY"} "http://127.0.0.1:$PORT/v1/models" >/dev/null 2>&1 && { echo "READY after ~$((i*5))s"; exit 0; }
  sleep 5
done

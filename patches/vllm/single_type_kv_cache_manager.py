...
class SlidingWindowManager(SingleTypeKVCacheManager):
    ...
    def find_longest_cache_hit(
        ...
        match_found = False
        _dbg_hits = 0
        _dbg_first_skip = None
        _dbg_last_cached_i = None
            ...
            if cached_block := block_pool.get_cached_block(
                ...
                if num_contiguous_blocks == 0 and block_size != alignment_tokens:
                    ...
                    if (post_pop_blocks * block_size) % alignment_tokens != 0:
                        if _dbg_first_skip is None:
                            _dbg_first_skip = (i, post_pop_blocks, alignment_tokens)
                        continue
                _dbg_last_cached_i = i
                ...
                num_contiguous_blocks = 0
        logger.info(
            "SWDBG win=%s bs=%s max_nb=%s need=%s eagle=%s align=%s "
            "last_cached_i=%s first_skip=%s match=%s",
            getattr(kv_cache_spec, "sliding_window", "?"), block_size,
            max_num_blocks, sliding_window_contiguous_blocks, drop_eagle_block,
            alignment_tokens, _dbg_last_cached_i, _dbg_first_skip, match_found,
        )
        if not match_found:

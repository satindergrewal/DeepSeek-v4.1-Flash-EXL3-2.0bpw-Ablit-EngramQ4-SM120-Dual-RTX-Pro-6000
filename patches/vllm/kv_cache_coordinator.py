...
class KVCacheCoordinator(ABC):
    ...
    def __init__(
        ...
        self.eagle_group_ids: set[int] = {
            ...
            # A sliding-window manager frees blocks as they leave the window, so
            # only ~window/block_size blocks remain cached at the end of a request.
            # The eagle +1 contiguous requirement is therefore unsatisfiable for SW
            # groups -> every prefix-cache hit returns 0 and caching is silently
            # dead for the whole model (measured: hits_total 0 with dspark on).
            if g.is_eagle_group and not isinstance(g.kv_cache_spec, SlidingWindowSpec)
        }
        # Conservatively fall back to flag all groups when no group is flagged.
        # EXCEPT sliding-window groups. Their manager frees blocks as they leave
        # the window, so only ~window/block_size blocks are still cached at the
        # end of a request; adding the eagle +1 to _contiguous_blocks_for_hit makes
        # the requirement unsatisfiable, every prefix-cache hit returns 0, and
        # caching is silently dead for the whole model. Measured on DeepSeek
        # V4.1-Flash + dspark: identical 52,846-token prompt = 28.9s then 26.9s
        # with hits_total==0; with the sliding-window groups excluded from the
        # eagle drop the requirement stays at 2 and hits are honoured.
        ...
        logger.info("EAGLEDGB eagle_group_ids=%s (of %d groups)", sorted(self.eagle_group_ids), len(kv_cache_config.kv_cache_groups))
    ...
    def find_longest_cache_hit(
        ...
        while True:
            ...
            for idx, (spec, group_ids, manager_cls, use_eagle) in enumerate(
                ...
                logger.info(
                    "CACHEDBG spec=%s groups=%s max_len=%d hit=%d (prefix_cacheable=%s)",
                    type(spec).__name__, group_ids, _max_length, _new_hit_length,
                    getattr(spec, "prefix_cacheable", "n/a"),
                )

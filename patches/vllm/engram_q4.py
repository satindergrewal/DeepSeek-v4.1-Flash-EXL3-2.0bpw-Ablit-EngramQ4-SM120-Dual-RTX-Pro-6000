...
def _engram_head_shard_weight_loader(
    ...
    # MXINT-4 packs store scales as e4m3; both fp8 dtypes are byte blobs to us.
    if loaded_weight.dtype in (torch.float8_e8m0fnu, torch.float8_e4m3fn):
...
def _engram_lookup_kernel(
    ...
    GRID: tl.constexpr,
    PACKED: tl.constexpr,
    PACKED3: tl.constexpr,
    INV_PRESCALE: tl.constexpr,
):
    ...
    for base in tl.range(tl.program_id(0) * BLOCK_R, num_rows, GRID * BLOCK_R):
        ...
        if PACKED:
            # MXINT-4: one byte holds column 2j in the low nibble, 2j+1 in the high.
            half = DIM // 2
            pcols = tl.arange(0, half)
            packed = tl.load(
                weight + local[:, None] * half + pcols[None, :],
                mask=owned[:, None],
                other=0,
            ).to(tl.uint8)
            lo = (packed & 0xF).to(tl.int8)
            hi = ((packed >> 4) & 0xF).to(tl.int8)
            lo = tl.where(lo >= 8, lo - 16, lo).to(tl.float32)
            hi = tl.where(hi >= 8, hi - 16, hi).to(tl.float32)
            values = tl.interleave(lo, hi)
            sc = tl.load(
                scales + local[:, None] * (DIM // QUANT_BLOCK) + scale_cols[None, :],
                mask=owned[:, None],
                other=0,
            ).to(tl.uint8)
            # e4m3 byte -> fp32, then undo the writer's prescale.
            scale = sc.to(tl.float8e4nv, bitcast=True).to(tl.float32) * INV_PRESCALE
        elif PACKED3:
            # MXINT-3: 8 three-bit codes bit-packed little-endian into 3 bytes.
            # Column j lives at bit 3j; a code may straddle two bytes.
            row_bytes = DIM * 3 // 8
            j = tl.arange(0, DIM)
            bit = j * 3
            bi = bit >> 3
            sh = (bit & 7).to(tl.int32)
            wrow = local[:, None] * row_bytes
            b0 = tl.load(
                weight + wrow + bi[None, :], mask=owned[:, None], other=0
            ).to(tl.int32)
            b1 = tl.load(
                weight + wrow + bi[None, :] + 1, mask=owned[:, None], other=0
            ).to(tl.int32)
            # (b1 << (8-sh)) contributes only when the code straddles; for sh==0
            # the <<8 result has no low-3 bits so the final &7 masks it out.
            code = (((b0 >> sh[None, :]) | (b1 << ((8 - sh)[None, :]))) & 7).to(tl.int32)
            code = tl.where(code >= 4, code - 8, code)
            values = code.to(tl.float32)
            sc3 = tl.load(
                scales + local[:, None] * (DIM // QUANT_BLOCK) + scale_cols[None, :],
                mask=owned[:, None],
                other=0,
            ).to(tl.uint8)
            scale = sc3.to(tl.float8e4nv, bitcast=True).to(tl.float32) * INV_PRESCALE
        else:
            values = tl.load(
...
class ParallelEngramEmbedding(nn.Module):
    ...
    def __init__(
        ...
        block_size: int = int(__import__("os").environ.get("DSV41_ENGRAM_BLOCK", "32")),
        ...
        # MXINT-4 (lab): set DSV41_ENGRAM_QBITS=4 when the pack's tables are packed.
        import os as _os
        self.qbits = int(_os.environ.get("DSV41_ENGRAM_QBITS", "0"))
        self.prescale = float(_os.environ.get("DSV41_ENGRAM_PRESCALE", "8.0"))
        ...
        self.weight = nn.Parameter(
            torch.empty(
                ...
                (dim // 2 if self.qbits == 4 else
                 dim * 3 // 8 if self.qbits == 3 else dim),
                dtype=(torch.uint8 if self.qbits in (3, 4)
    ...
    def lookup(
        ...
        _engram_lookup_kernel[(grid,)](
            ...
            GRID=grid,
            PACKED=self.qbits == 4,
            PACKED3=self.qbits == 3,
            INV_PRESCALE=1.0 / self.prescale,
        )

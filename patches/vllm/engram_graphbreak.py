...
def _engram_disk_lookup_op(
    indices: torch.Tensor,
    out: torch.Tensor,
    layer_name: str,
) -> None:
    """NVMe gather for the Engram tables, deliberately kept OUT of CUDA graphs.

    It contains a device->host sync and blocking preads, so it cannot be captured - the
    failure is "Cannot copy between CPU and CUDA tensors during CUDA graph capture".
    Registering it as a splitting op makes vLLM break the piecewise graph around it, the
    same treatment Qwen's PLE n-gram lookup gets (vllm::qwen4_exp_compute_ple_ngram_ids).

    Writes into `out`, which is graph-owned storage (Engram.staged_rows) allocated at model
    init - torch.compile requires a splitting op to write graph-owned storage rather than
    return a fresh tensor.
    """
    if os.environ.get("ENGRAM_NOOP") == "1":
        # TIMING CONTROL: keep the eager break, drop the NVMe read.
        out.zero_()
        return
    layer = get_forward_context().no_compile_layers[layer_name]
    layer._disk_lookup(indices, out)


def _engram_disk_lookup_op_fake(
    indices: torch.Tensor,
    out: torch.Tensor,
    layer_name: str,
) -> None:
...
direct_register_custom_op(
    op_name="engram_disk_lookup",
    op_func=_engram_disk_lookup_op,
    mutates_args=["out"],
    fake_impl=_engram_disk_lookup_op_fake,
)

ENGRAM_DISK_SPLITTING_OP = "vllm::engram_disk_lookup"
...
class ParallelEngramEmbedding(nn.Module):
    ...
    def __init__(
        ...
        block_size: int = int(os.environ.get("DSV41_ENGRAM_BLOCK", "32")),
        ...
        self._engram_layer_name: str | None = None
        ...
        if engram_disk_enabled():
            ...
            # Reachable from the op via get_forward_context().no_compile_layers, which IS
            # compilation_config.static_forward_context (vllm/forward_context.py:230).
            self._engram_layer_name = f"engram.{layer_id}.embed_tokens"
            _cc = get_current_vllm_config().compilation_config
            if self._engram_layer_name not in _cc.static_forward_context:
                _cc.static_forward_context[self._engram_layer_name] = self
            # Break the piecewise graph here so the host-side gather runs eagerly between
            # graph segments instead of being captured (and failing).
            _was_none = _cc.splitting_ops is None
            if not _was_none and ENGRAM_DISK_SPLITTING_OP not in _cc.splitting_ops:
                _cc.splitting_ops.append(ENGRAM_DISK_SPLITTING_OP)
            logger.info(
                "ENGRAM-BREAK diag: layer=%s splitting_ops_was_None=%s now_contains=%s n=%d",
                self._engram_layer_name,
                _was_none,
                ENGRAM_DISK_SPLITTING_OP in (_cc.splitting_ops or []),
                len(_cc.splitting_ops or []),
            )
    ...
    def lookup(
        ...
        if self.disk is not None:
            if self._engram_layer_name is not None:
                torch.ops.vllm.engram_disk_lookup(indices, out, self._engram_layer_name)
            else:

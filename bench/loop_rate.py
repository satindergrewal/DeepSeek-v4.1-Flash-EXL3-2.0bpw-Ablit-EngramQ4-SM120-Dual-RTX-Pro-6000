...
def generate(host, port, prompt, n_predict, sampler, timeout, temp=None, chat=False,
    ...
    # llama.cpp's server takes no key; vLLM is usually started with --api-key and would
    # 401 every request without this, which reads as "the model generated nothing".
    if api_key:
    ...
    if chat:
        ...
        # OpenAI-compatible servers (vLLM) report counts under `usage`, not llama.cpp's
        # `timings`. Without this the n_pred / depth_n_past columns read 0 against vLLM.
        # Additive only: the detector and the three metrics are untouched.
        timings = dict(payload.get("timings") or {})
        usage = payload.get("usage") or {}
        if usage.get("completion_tokens") is not None:
            timings["predicted_n"] = usage["completion_tokens"]
        if usage.get("prompt_tokens") is not None:
            timings["prompt_n"] = usage["prompt_tokens"]
        return text, timings, stop
...
def main():
    ...
    # --limit N runs only the first N prompts. Exists so a long run can be smoke-tested
    # end to end first: ast.parse does not exercise argparse, the imports, or the endpoint.
    ap.add_argument("--limit", type=int, default=0, help="run only the first N prompts (0 = all)")
    ap.add_argument("--api-key", default=os.environ.get("API_KEY", ""),
                    help="bearer token; defaults to $API_KEY. llama.cpp needs none.")
    ...
    for name, prompt, kind in (PROMPTS[: args.limit] if args.limit else PROMPTS):
        # timings must exist even when generate() raises: the depth branch below reads
        # timings["prompt_n"], so without this a SINGLE timeout kills an entire depth run
        # with UnboundLocalError instead of recording one ERR row.
        timings = {}
        try:
            text, timings, stop = generate(args.host, args.port, prefix + prompt,
                                           args.n_predict, args.sampler, args.timeout,
                                           args.temp, args.chat, args.api_key)
